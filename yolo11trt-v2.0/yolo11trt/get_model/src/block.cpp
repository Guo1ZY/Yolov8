...
{
    std::cout << "\033[32m" << "wts权重路径: " << file << "\033[0m" << std::endl;
    std::map<std::string, nvinfer1::Weights> WeightMap;

    std::ifstream input(file);
    if (!input.is_open())
    {
        std::cout << "\033[36m" << "?不是哥们，" << "\033[33m" << "选对路径" << "\033[31m" << "很难" << "\033[33m" << "吗?" << "\033[0m" << std::endl;
        assert(false);
    }
    ...
    if (count <= 0)
    {
        std::cout << "\033[35m" << "妈的你确定你给出的是一个" << "\033[31m" << "正确" << "\033[35m" << "的wts文件？" << "\033[0m" << std::endl;
        assert(false);
    }
