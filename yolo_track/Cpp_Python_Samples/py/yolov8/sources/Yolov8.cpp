 ...
 */
Yolov8::Yolov8(const std::string modelPath, const std::string classesPath)
{
    /*加载Yolov8模型*/
...
std::vector<YoloDetect> Yolov8::detect(const cv::Mat &image)
{
    ...
    for (Py_ssize_t i = 0; i < numSublists; i++)
    {
        ...
        if (!PyList_Check(sublist) || PyList_Size(sublist) != 7)
        {
            COUT_RED_START
        ...
        // 转换 id 形式的值（只有在使用Track时才不用注释掉）
        float id = PyFloat_AsDouble(PyList_GetItem(sublist, 6));
...
void Yolov8::drawResult(cv::Mat &image, const std::vector<YoloDetect> &results)
{
    for (YoloDetect result : results)
    {
        ...
        std::string label = cv::format("%d", result.id) +"  "+result.class_name + ":" + cv::format("%.2f", result.confidence);
...
void Yolov8::loadYolov8Model(const std::string modelPath)
{
    ...
    case YoloMode::Track:
        pyModule = PyImport_ImportModule("Yolov8_Track");
        break;
    default:
        break;
    }
    ...
    case YoloMode::Track:
        pyYolov8Class = PyObject_GetAttrString(pyModule, "Yolov8_Track");
        break;
    ...
    pyYolov8Detect = PyObject_GetAttrString(pyYolov8Instance, "track_t");
